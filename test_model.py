    ...
    for vol_f in train_vol_list:
            ...
            while not coord.should_stop():
                try:
                    ...
                    #tmp_img
                    next_patch = np.zeros(shape=([1,1,14,14]))
                    for x in range(14):
                        for y in range(14):
                            np.insert(next_patch, x+y*14, tmp_img.GetPixel(x,y,0))

                    ap_eval = sess.run([acctual_prediction], feed_dict={x: next_patch, y: batch_y_eval})
