...
def prepare_data(vol_path, seg_path, vol_dest_path, seg_dest_path, klr,SegRatio,
    ...
    for f in vol_list:
        ...
        left_kidney_seg = dh.Seg_size_reduce(left_kidney_seg.astype(np.float32), r, start,
        ...
        right_kidney_seg = dh.Seg_size_reduce(right_kidney_seg.astype(np.float32), r, start,
        ...
        while (g_r_count < dxy and r_count < 20):
            ...
            if f:  # if used before go to next itteration with advancing counters
                ...
                for i in range(s[0]):
                    if (((shifted_input_vol[i, :, :] < 10).all()) or ((shifted_input_vol[i, :, :] > 400).all()) or \
                                (np.sum(shifted_liver_seg[i, :, :]) > 0 and np.sum(
                                    shifted_liver_seg[i, :, :]) < SegRatio * patch_size ** 2) or \
                                (np.sum(shifted_kidney_seg[i, :, :]) > 0 and np.sum(
                                    shifted_kidney_seg[i, :, :]) < SegRatio * patch_size ** 2)):
                        ind_del.append(i)
                ...
                if kidney_count / float(liver_count) < klr / float(100):
                    ...
                    print(str(kidney_count) + " kidney patches, " + str(liver_count) + " liver patches and " + str(nothing_count) + " the rest")
